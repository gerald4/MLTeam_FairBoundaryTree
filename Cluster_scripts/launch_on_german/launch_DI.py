...
idx = int(sys.argv[1])
config = [{'n_leaves':15, 'impurity':'entropy', 'n_trees':50, 'n_samples':150}][idx-1]
...
path = os.path.join('./results_german_DI', str(n_leaves) + '_' + impurity + '_' + str(n_trees))
if not os.path.exists(path):
    os.makedirs(path)

for i in range(10,31):

    X = np.load('X_german.npy').astype(np.double)
    y = np.load('y_german.npy').astype(np.intp)
