...
unfair_X[:,1:3] = X[:,:] ; unfair_X[:,0] = x_control[:]
y[np.where(y == -1)] = 0
unfair_X = unfair_X.astype(np.double)
y = y.astype(np.intp)

idx = int(sys.argv[1])
config = [{'n_leaves':15, 'impurity':'entropy', 'n_trees':50, 'n_samples':500}][idx-1]
...
path = os.path.join('./results_synthetic_DI', str(n_leaves) + '_' + impurity + '_' + str(n_trees))
if not os.path.exists(path):
    os.makedirs(path)

for i in range(10,31):

    X_train, X_test, y_train, y_test = train_test_split(unfair_X, y, test_size=0.25, shuffle=True, random_state=i)
