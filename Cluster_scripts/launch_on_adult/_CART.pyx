...
cdef class Tree:
    ...
    cpdef str print_struct(self):
        ...
        for i in range(self.node_count):
            ...
            structure += '#'.join((str(i), str(node.parent), str(node.left_child), str(node.right_child), 
                                   str(node.feature), str(node.threshold), str(node.impurity), str(node.gain), str(node.value))) + ';'

        return structure[:-1]


    cdef void _add_node(self, SIZE_t parent, bint is_left, bint is_leaf, SIZE_t feature, 
                        double threshold, double impurity, double gain, SIZE_t value) nogil:
    ...
    cdef void _update_node(self, SIZE_t node_id, SIZE_t feature, double gain, double threshold) nogil:
    ...
    cpdef void fit(self, np.ndarray[DOUBLE_t, ndim=2] X, np.ndarray[SIZE_t, ndim=1] y, list features):
        """
        Build the tree.
        ...
        cdef:
            ...
            double gain, pure_gain, weight, unfairness
            Node* node
            Tree _tree
            double[:] stats
            double[:] X_feat
            ...
            double _best_pure_gain, best_pure_gain
        ...
        # --> self_add_node(parent, is_left, is_leaf, feature, threshold, impurity, gain, value)
        self._add_node(_TREE_UNDEFINED, _TREE_UNDEFINED, 1, _TREE_UNDEFINED, INFINITY, impurity, 0., value)
        leaf_nodes.append(0)
        ...
        while ( self.node_count < (2*self.max_leaves-1) ) and len(splittable_nodes) != 0:
            ...
            for node_id in splittable_nodes:
                ...
                if _best_gain > best_gain:
                    ...
                    best_pure_gain = _best_pure_gain
            ...
            self._update_node(best_node, best_feature, best_pure_gain, best_threshold)
            ...
            # Left child node
            self._add_node(best_node, 1, 1, _TREE_UNDEFINED, INFINITY, best_impurity_1, 0., best_value_1)
            ...
            # Right child node
            self._add_node(best_node, 0, 1, _TREE_UNDEFINED, INFINITY, best_impurity_2, 0., best_value_2)
    ...
    cdef double get_DI_cov(self, double[:,:] X, list leaves, SIZE_t split, double threshold, 
        ...
        cdef:
            ...
            SIZE_t i, j
            Node* node
            double z_bar, DI, dist, curr_dist
    ...
    cdef double get_DI_corr(self, double[:,:] X, int[:] leaves, SIZE_t split, double threshold, 
                            SIZE_t feature, SIZE_t value_1, SIZE_t value_2):
    ...
    cpdef np.ndarray[DOUBLE_t, ndim=1] get_DT_features_count(self, n_features):
        ...
        count = np.zeros((n_features,), dtype=np.double)
        for i in range(self.node_count):
            node = &self.nodes[i]
            feature = node.feature
            if feature != -2:
                count[feature] += node.gain
