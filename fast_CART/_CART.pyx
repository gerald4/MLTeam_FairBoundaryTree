...
cdef class Tree:
    ...
    def __cinit__(self, SIZE_t min_samples, SIZE_t max_leaves, double _lambda, str impurity_type='gini'):
        """Initialization."""
    ...
    cpdef void fit(self, np.ndarray[DOUBLE_t, ndim=2] X, np.ndarray[SIZE_t, ndim=1] y):
        ...
        cdef:
            ...
            Node* node
            Tree _tree
            np.ndarray[SIZE_t, ndim=1] labels
            np.ndarray[SIZE_t, ndim=1] stats
            ...
            np.ndarray[double, ndim=1] X_feat
            np.ndarray[double, ndim=2] X_in_node

        # Building the root node 
        labels, stats = np.unique(y, return_counts=True)
        value = labels[np.argmax(stats)]
        
        if self.impurity_type == 'entropy':
            impurity = self.entropy(stats)
        else:
            impurity = self.gini(stats)
        ...
        while ( self.node_count < (2*self.max_leaves-1) ) and len(splittable_nodes) != 0:
            ...
            for node_id in splittable_nodes:
                ...
                for feature in range(n_features):
                    X_feat = np.sort(X_in_node[:,feature])

                    if X_feat[-1] <= X_feat[0] + EPSILON:
                        continue
                    ...
                    while p < end:
                        ...
                        if p < end: 
                            ...
                            set1 = np.where((X[:,feature] <= X_feat[p-1]) & (in_node[:] == node_id))[0]
                            set2 = np.where((X[:,feature] >  X_feat[p-1]) & (in_node[:] == node_id))[0]

                            # Copy the tree and add the current split in order to evaluate it
                            _tree = deepcopy(self)

                            # Left child node
                            labels, stats = np.unique(y[set1], return_counts=True)
                            value_1 = labels[np.argmax(stats)]
                            if self.impurity_type == 'entropy':
                                impurity_1 = self.entropy(stats)
                            else:
                                impurity_1 = self.gini(stats)
                            _tree._add_node(node_id, 1, 1, _TREE_UNDEFINED, INFINITY, impurity_1, value_1)
                            
                            # Right child node
                            labels, stats = np.unique(y[set2], return_counts=True)
                            value_2 = labels[np.argmax(stats)]
                            if self.impurity_type == 'entropy':
                                impurity_2 = self.entropy(stats)
                            else:
                                impurity_2 = self.gini(stats)
                            _tree._add_node(node_id, 0, 1, _TREE_UNDEFINED, INFINITY, impurity_2, value_2)

                            # Computing how good this split is
                            weight = set1.shape[0]/(set1.shape[0] + set2.shape[0])
                            gain = node.impurity - weight*impurity_1 - (1-weight)*impurity_2
                            if gain > _best_gain:
                                _best_gain = gain
            ...
            # Update the tree by performing the best split
            ...
            print(best_node, best_gain, best_feature, best_threshold)

        return


    #cpdef np.ndarray predict(self):
