...
class Node:
	...
	def __init__(self, index, index_parent, left_node, right_node, samples_index, y_unique, stats, table1, table2, feature = -2, threshold = - np.inf):
		self.index = index
...
def unfairness( table1, table2, type = "DI"):
	if table1.shape[0]==0:
		table1 = np.array([0])
	if table2.shape[0]==0:
		table2 = np.array([0])
	
	return np.abs(table1[0]/(np.sum(table1) + EPS) - table2[0]/(np.sum(table2)) + EPS)
...
class DecisionTreeFAHT:
	""" This a binary decision tree built from BestFirstSearch """
	def __init__(self, min_samples, impurity_type, _lambda, max_leaves = 1000, fair_type = "DI"):
	...
	def fit(self, X, y):
		...
		table1 = np.bincount(y[np.where(X[:,-1] == 0)[0]])
		table2 = np.bincount(y[np.where(X[:,-1] == 1)[0]])

		self.M = X.shape[1]
		#Tree initialisation
		root_node = Node(self.counter, -1, self.counter + 1, self.counter + 2, np.arange(X.shape[0]), y_unique, stats, table1, table2)
		self.counter += 3
		...
		while len(self.stack_nodes)!=0 and len(self.nodes)  < 2 * self.max_leaves - 1 and drap: 
			...
			for node in self.unvisited.values(): 
				...
				current_unfairness = unfairness(node.table1, node.table2)
				# if current_unfairness !=0:
				# 	current_score = current_score*current_unfairness
						...
						if X_feat[-1] <= X_feat[0] + FEATURE_THRESHOLD:
							...
							while p < end:
								...
								if p < end:
									...
									indexes1 = node.samples_index[set1]
									indexes2 = node.samples_index[set2]

									table11 = np.bincount(y[indexes1][np.where(X[indexes1][:,-1] == 0)[0]])

									table12 = np.bincount(y[indexes1][np.where(X[indexes1][:,-1] == 1)[0]])

									table21 = np.bincount(y[indexes2][np.where(X[indexes2][:,-1] == 0)[0]])

									table22 = np.bincount(y[indexes2][np.where(X[indexes2][:,-1] == 1)[0]])

									if current_unfairness - weight*unfairness(table11, table12) - (1-weight)*unfairness(table21,table22)!=0:
										gain = (current_score - weight * self.impurity_func(node1_stats) - (1-weight)*self.impurity_func(node2_stats))*(current_unfairness - weight*unfairness(table11, table12) - (1-weight)*unfairness(table21,table22))
									else:
										gain =  current_score - weight * self.impurity_func(node1_stats) - (1-weight)*self.impurity_func(node2_stats)				
			...
			if best_gain_for_nodes > 0:
				...
				node1 = Node(node.left_node, node.index, self.counter, self.counter + 1, 
													node.samples_index[set1], node1_y_unique, node1_stats, table11,table12)
				node2 = Node(node.right_node, node.index, self.counter + 2, self.counter + 3,
													node.samples_index[set2], node2_y_unique, node2_stats, table21, table22)
